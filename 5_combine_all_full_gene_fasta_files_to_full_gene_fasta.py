...
3. Combines them in order into a new FASTA file called full_gene.fasta
"""
...
def extract_and_combine_fasta(directory_path, output_file="full_gene.fasta"):
    """
...
def main():
    ...
    parser.add_argument(
        "-o", "--output", 
        default="full_gene.fasta",
        help="Name of output file (default: full_gene.fasta)"
    )
    args = parser.parse_args()
    
    print("FASTA Sequence Combiner")
    print("======================")
    print("\nThis script will combine all sequences from FASTA files in a directory into a single file called 'full_gene.fasta'.")
    
    # Always ask for directory path, regardless of command line arguments
    print("\nIMPORTANT: You need to provide the directory(folder) containing your full gene FASTA files.")
    default_dir = args.directory if args.directory else os.getcwd()
    print(f"Default directory: {default_dir}")
    directory_path = input("Enter the path to the directory(folder) containing full gene FASTA files: ").strip()
