...
echo "with '_protein' added to their filenames."
echo "======================================"
...
for fasta_file in "$FASTA_DIR"/*.fa "$FASTA_DIR"/*.fasta "$FASTA_DIR"/*.fna; do
  ...
  # Create new filename with "_protein" added
  new_filename="${filename_noext}__protein.${extension}"
...
if [ $processed -eq 0 ]; then
  ...
  echo "Files now have '__protein' suffix."
