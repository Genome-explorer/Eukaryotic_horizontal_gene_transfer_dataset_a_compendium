...
3. Updates the 10th column of the CSV file sequentially without matching
"""
...
def update_csv_with_gc_content_sequentially(csv_file, gene_data):
    """
    Update the CSV file with GC content in the 10th column sequentially,
    ...
    # Ensure each row has at least 10 columns
    for i in range(len(rows)):
        while len(rows[i]) < 10:
            rows[i].append("")
    
    # Update the 10th column (index 9) with GC content sequentially
