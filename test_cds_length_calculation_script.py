...
4. Creates a new FASTA file with these sequences, separated by ###############
"""
...
def create_fasta_index(fasta_file):
    ...
    Also calculates gene length for each sequence, ignoring N characters.
    
    Args:
        fasta_file (str): Path to the FASTA file
        
    Returns:
        dict: Dictionary mapping accession numbers to sequences with length info
    """
    index = {}
    pattern = r'^>([^|\s]+)'
    ...
    try:
        with open(fasta_file, 'r') as f:
            ...
            for line in f:
                line = line.strip()
                if line.startswith('>'):
                    # If we were collecting a sequence, calculate length and save it
                    if current_accession and current_header and sequence_content:
                        ...
                        # Calculate length (ignoring N characters)
                        sequence_without_n = ''.join(c for c in complete_sequence.upper() if c != 'N')
                        length = len(sequence_without_n)
                        # Add length to header
                        modified_header = f"{current_header} This is the CDS Length: {length}"
                    ...
                    # Also keep track of the actual sequence content for length calculation
...
def create_output_fasta(selected_accessions, fasta_file, output_file):
    ...
    """
    # Create an index that stores the sequences
...
def main():
    ...
    """
    print("Selective FASTA Sequence Extractor")
    print("==================================")
    ...
    output_file = "Test_CDS_calculated_sequences.fasta"
