...
echo "with '_full_gene' added to their filenames."
echo "======================================"
...
for fasta_file in "$FASTA_DIR"/*.fa "$FASTA_DIR"/*.fasta "$FASTA_DIR"/*.fna; do
  ...
  # Create new filename with "full_gene" added
  new_filename="${filename_noext}_full_gene.${extension}"
...
if [ $processed -eq 0 ]; then
  ...
  echo "Files now have '_full_gene' suffix."
